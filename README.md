...
- **状态驱动调度**：后台持续轮询目标状态；`paused` 会触发恢复，`active` 只观察不重复发消息，`complete` 会把自动任务标为已完成并停止后续调度。
- **失败恢复**：网络/超时/502/503/504 默认按指数退避持续重试（最大退避 6 小时；将最大重试次数设为大于 0 可限制次数）；429、quota、usage limit 会挂起计划。官方订阅优先等待接口提供的 `reset_at`，等待期间仍定期检查目标与额度，若提前恢复就立即继续；没有重置时间时按额度轮询间隔重试。
